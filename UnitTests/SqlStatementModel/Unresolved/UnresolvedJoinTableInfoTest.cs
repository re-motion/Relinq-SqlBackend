...
{
  [TestFixture]
  public class UnresolvedJoinTableInfoTest
  {
    ...
    public void Initialization_CardinalityMany_NonEnumerable_Throws ()
    {
      ...
      Assert.That (
          () => new UnresolvedJoinTableInfo (_entityExpression, typeof (Cook).GetProperty ("Substitution"), JoinCardinality.Many),
    ...
    {
      var joinInfo = new UnresolvedJoinTableInfo (_entityExpression, typeof (Cook).GetProperty ("Substitution"), JoinCardinality.One);
    ...
    public void ItemType_CardinalityMany ()
    {
      ...
      var joinInfo = new UnresolvedJoinTableInfo (entityExpression, typeof (Restaurant).GetProperty ("Cooks"), JoinCardinality.Many);
      Assert.That (joinInfo.ItemType, Is.SameAs (typeof (Cook)));
    }
    ...
    public void Accept ()
    {
      ...
      tableInfoVisitorMock.Verify(mock => mock.VisitUnresolvedJoinTableInfo (tableInfo));
    }
    ...
    {
      var tableInfo = SqlStatementModelObjectMother.CreateUnresolvedJoinTableInfo_KitchenCook();
      var result = tableInfo.ToString ();
      Assert.That (result, Is.EqualTo ("TABLE(Kitchen.Cook)"));
    }
  }
}