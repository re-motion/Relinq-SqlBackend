  ...
  public class DefaultMappingResolutionStage : IMappingResolutionStage
  {
    ...
    public virtual SqlEntityExpression ResolveEntityRefMemberExpression (SqlEntityRefMemberExpression expression, IMappingResolutionContext context)
    {
      ...
      var join = originatingSqlTable.GetOrAddLeftJoinByMember (
          ...
          () =>
          {
            // Note that it's important to not return a join with unresolved components here because the new join might not go through 
            // SqlStatementResolver any more. (The originatingSqlTable might have already been resolved.)
            ...
            var joinedTable = new SqlTable (resolvedTableInfo);
            var unresolvedJoinCondition = new UnresolvedJoinConditionExpression (expression.OriginatingEntity, expression.MemberInfo, joinedTable);
            var joinCondition = ResolveJoinCondition (unresolvedJoinCondition, context);
