  ...
  public class OrderByClausesSqlBackendIntegrationTest : SqlBackendIntegrationTestBase
  {
    ...
    public void AutomaticOrderByHandlingInSubStatements_InSelectClause_WithTopExpression ()
    {
      CheckQuery (
         ...
         "SELECT [q3].[ID],[q3].[CookID],[q3].[Name],[q3].[RestaurantID],[q3].[SubKitchenID] FROM [CookTable] AS [t0] OUTER APPLY "
          +"(SELECT TOP (1) [t1].[ID],[t1].[CookID],[t1].[Name],[t1].[RestaurantID],[t1].[SubKitchenID] FROM [KitchenTable] AS [t1] LEFT OUTER JOIN "
