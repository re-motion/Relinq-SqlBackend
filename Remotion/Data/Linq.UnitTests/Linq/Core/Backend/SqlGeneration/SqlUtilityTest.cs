...
namespace Remotion.Data.Linq.UnitTests.Linq.Core.Backend.SqlGeneration
{
  ...
  public class SqlUtilityTest
  {
    ...
    {
      IDbCommand command = SqlUtility.CreateCommand ("SELECT [s].* FROM [cookTable] [s]", new CommandParameter[0], _databaseInfo, _connection);
      Assert.IsNotNull (command);
      Assert.AreEqual ("SELECT [s].* FROM [cookTable] [s]", command.CommandText);
    ...
    {
      IDbCommand command = SqlUtility.CreateCommand ("SELECT [s].* FROM [cookTable] [s] WHERE @foo=@1",
          new CommandParameter[] { new CommandParameter("@1", "Garcia"), new CommandParameter("@foo", "bla")}, _databaseInfo, _connection);
      
      Assert.IsNotNull (command);
      Assert.AreEqual ("SELECT [s].* FROM [cookTable] [s] WHERE @foo=@1", command.CommandText);
      Assert.AreEqual (CommandType.Text, command.CommandType);
