...
namespace Remotion.Data.Linq.UnitTests.Backend.DataObjectModel
{
  ...
  public class DatabaseInfoUtilityTest
  {
    ...
    {
      Assert.That (StubDatabaseInfo.Instance.IsRelationMember (typeof (Cook).GetProperty ("FirstName")), Is.False);
    }

    [Test]
    public void IsRelationMember_NonDBMember ()
    {
      Assert.That (StubDatabaseInfo.Instance.IsRelationMember (typeof (Cook).GetProperty ("NonDBStringProperty")), Is.False);
    }
