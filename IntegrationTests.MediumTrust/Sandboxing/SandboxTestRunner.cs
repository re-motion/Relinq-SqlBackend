...
namespace Remotion.Linq.SqlBackend.IntegrationTests.MediumTrust.Sandboxing
{
  /// <summary>
  ...
  public class SandboxTestRunner : MarshalByRefObject
  {
    ...
    public TestFixtureResult RunTestFixture (Type type)
    {
      ...
      var setupMethod = type.GetMethods ().SingleOrDefault (m => IsDefined (m.GetBaseDefinition(), "NUnit.Framework.SetUpAttribute"));
      var tearDownMethod = type.GetMethods ().SingleOrDefault (m => IsDefined (m.GetBaseDefinition(), "NUnit.Framework.TearDownAttribute"));
