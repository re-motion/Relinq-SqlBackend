...
namespace Rubicon.Data.Linq.UnitTests.SqlGenerationTest.SqlServer
{
  ...
  public class WhereBuilderTest
  {
    ...
    [Test]
    [Ignore ("TODO: Obsolete with new implementation of virtual side conditions")]
    public void VirtualSide_EqualNull ()
    {
      // industrialSector.StudentDetail == null
      StringBuilder commandText = new StringBuilder ();
      List<CommandParameter> parameters = new List<CommandParameter> ();
      WhereBuilder whereBuilder = new WhereBuilder (commandText, parameters);
    ...
    [Test]
    [Ignore ("TODO: Obsolete with new implementation of virtual side conditions")]
    public void VirtualSide_EqualValue ()
    {
      // industrialSector.StudentDetail == studentDetail
      StringBuilder commandText = new StringBuilder ();
      List<CommandParameter> parameters = new List<CommandParameter> ();
      WhereBuilder whereBuilder = new WhereBuilder (commandText, parameters);
      Student_Detail studentDetail = new Student_Detail();
      studentDetail.ID = 42;
      ICondition condition = GetVirtualSideCondition (BinaryCondition.ConditionKind.Equal, new Constant (studentDetail));

      whereBuilder.BuildWherePart (condition);

      Assert.AreEqual
          (" WHERE EXISTS (SELECT 1 FROM [detailTable] [xyz] WHERE [xyz].[Student_Detail_to_IndustrialSector_FK] = [is].[IndustrialSector_PK] AND [xyz].[Student_Detail_PK] = @1)",
              commandText.ToString ());
      Assert.That (parameters, Is.EqualTo (new[] { new CommandParameter ("@1", 42) }));
    }

    [Test]
    [Ignore ("TODO: Obsolete with new implementation of virtual side conditions")]
    public void VirtualSide_NotEqualValue ()
    {
      Assert.Fail ("TODO");
    }
